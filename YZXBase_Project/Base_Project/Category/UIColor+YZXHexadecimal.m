...
#import "UIColor+YZXHexadecimal.h"

@implementation UIColor (YZXHexadecimal)

+ (UIColor *)hexColor:(NSString *)hex {
    return [UIColor hexColor:hex alpha:1.0];
}

+ (UIColor *)hexColor:(NSString *)hex alpha:(double)alpha
{
    // Remove `#` and `0x`
    if ([hex hasPrefix:@"#"]) {
